    ...
    try {
      ...
      if (Array.isArray(workspaces)) {
        ...
        const activeWorkspaces = workspaces.filter(ws => {
          const lockFile = path.join(ws, '.wechat-agent', '.ide.lock');
      ...
      // If the current active workspace is no longer in the list, fall back to the first available one
      if (activeWorkspace && !workspaces.some(w => path.resolve(w).toLowerCase() === path.resolve(activeWorkspace).toLowerCase())) {
        activeWorkspace = workspaces.length > 0 ? workspaces[0] : '';
        try {
          fs.writeFileSync(activeWsFile, activeWorkspace, 'utf-8');
        } catch (e) {}
      }
