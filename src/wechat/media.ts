...
/**
 * Download a CDN image, decrypt it, and save to a temp directory.
 * Returns the local file path, or null on failure.
 */
export async function saveImageToFile(item: MessageItem): Promise<string | null> {
  const imageItem = item.image_item;
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    logger.warn('Failed to save image', { error: msg });
    return null;
  }
}
...
/**
 * Download a CDN voice message, decrypt it, and save to a temp directory.
 * Returns the local file path, or null on failure.
 */
export async function downloadVoice(item: MessageItem): Promise<string | null> {
  ...
  if (voiceItem.media?.encrypt_query_param) {
    encryptQueryParam = voiceItem.media.encrypt_query_param;
    aesKey = voiceItem.media.aes_key;
  }
  ...
  try {
    ...
    const filePath = path.join(tmpDir, fileName);
    fs.writeFileSync(filePath, decrypted);
    logger.info('Voice file downloaded and saved', { path: filePath, size: decrypted.length });
    return filePath;
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    logger.warn('Failed to download voice file', { error: msg });
    return null;
  }
}
